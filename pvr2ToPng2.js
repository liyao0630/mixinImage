...
var path = require('path')
var entry = __dirname + '/Res/apple/'
var output = __dirname + '/Res/apple/'
...
function splitPrvToPng(pvr) {
  pvr.forEach(async (files, index) => {
    ...
    var res = execSync(`/usr/local/bin/TexturePacker ${pngPath} --data ${getOutputPlist(Date.now() + '__.plist')} --texture-format pvr2 --format corona-imagesheet --shape-padding 0 --border-padding 0 --padding 0 --sheet ${prvPath} --opt PVRTC2`)
    unlinkPng(pngPath)
...
///Applications/Xcode.app/Contents/Developer/Platforms/iPhoneOS.platform/Developer/usr/bin/texturetool -e PVRTC -o 56200.pvr -f PVR 56200.png
